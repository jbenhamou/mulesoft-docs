...
. Bookstore calls Tweetbook’s resource server to request the user’s protected resource. It offers the token Tweetbook provided as the resource owners's authorization to acquire the resource on its behalf.
+
